...
from typing import Callable, List, Type, Optional, Union, Tuple, Any, Dict
...
class CRUDJsonEndpoints:
    ...
    Requires:
        ...
        - collection: Beanie Document class for the database collection. 
        # TODO default to PyMongo database types, using DragonApp DatabaseMongoBaseModel schema.
        # TODO - Add support for SQLModel
        # TODO - add a Schema endpoint, to return the schema of the collection.
        # TODO - when inserting/saving, and value type should be reference to another model, attempt to save the reference first, and then save the item. If the reference fails, then the item should not be saved. Relationship updating!
    ...
    def __init__(
        ...
        collection: Union[List[Type[Document]], Type[Document]] = None,    
        prefix: Optional[str] = "",
        method: Union[str, List[str]] = ["GET","POST","PATCH","DELETE"], 
        description: Optional[str] = None,
        name_plural: Optional[str] = None, # TODO deprecate singular? What to do if multiple collections are provided? Use Settings.name? or Collection.__name__?
        ...
        # Callbacks
        pre_query_callback: Optional[Callable] = None,
        post_query_callback: Optional[Callable] = None,
        pre_output_callback: Optional[Callable] = None,
        post_output_callback: Optional[Callable] = None,
        ...
        Args:
            router: FastAPI APIRouter instance to attach the endpoints to.
            base_name: Base name for the collection. Prefixed to collection tags and names. Default is None.
            collection: List or single DragonMongoDatabaseBaseModel class to create the endpoints for.
            prefix: Prefix for the endpoint URL. Default is "".
            method: HTTP method to create the endpoints for. Can be a string or a list of strings. Default is ["GET","POST","PATCH","DELETE"].
            description: Description for the endpoint. Default is None.
            name_plural: Plural name for the collection items. 
            
            input_model: Default Pydantic model for the input data, used for POST and PATCH queries. Default is the Collection model.
            output_model: Default Pydantic model for the output data, used for GET queries. Default is Collection model.
            
            dependencies: List of FastAPI Depends functions to execute before the endpoint is executed. Default is None.
            tags: List of tags to apply to the endpoint. Default is None.
            cache_ttl: Time to live for the cache. Default is None.
            
            pre_query_callback: Function to execute before the database query is made. Default is None.
            post_query_callback: Function to execute after the database query is made. Default is None.
            pre_output_callback: Function to execute before the output is sent. Default is None.
            post_output_callback: Function to execute after the output is sent. Default is None.
            
            
            **kwargs: Additional parameters to set the default values for the APIRouter. See https://fastapi.tiangolo.com/reference/apirouter/#fastapi.APIRouter
        """
        
        
        self.router = router
        self.prefix = prefix.rstrip("/")
        self.base_name = base_name if base_name else ""
        ...
        if output_model and "id" not in self.output_model.model_dump():
        ...
        # Callbacks
        self.pre_query_callback = pre_query_callback 
        self.post_query_callback = post_query_callback 
        self.pre_output_callback = pre_output_callback 
        self.post_output_callback = post_output_callback 
    ...
    def build(self,
        ...
        # DEBUG Ignore name_plural/name_singluar
        # if not name_singluar and not name_plural:
            # raise ValueError("At least one of name_singluar or name_plural must be provided")
        # name_plural = collection.__name__.lower() + "s" # Moved to below defining the collection
        name_singluar = None # self.collection.__name__.lower() # using only plural names for now
        ...
        if not tags and name_singluar: tags = [(self.base_name + " " if self.base_name else "") + name_singluar]
        ...
        if isinstance(collection, list):
            for c in collection:
                ...
                self.build(
                    ...
                    return_item_data_on_error=return_item_data_on_error,
        ...
        if not name_plural: 
            try:
                name_plural = collection.Settings.item_name_plural
            except:
                name_plural = collection.__name__.lower() + "s"
        
        name_singluar = name_plural
        
        # Input/Output Models and Callbacks
        if output_model and "id" not in output_model.model_dump():
            raise ValueError("Output model must have an ID field")
        ...
        #### SCHEMA ####
        # endpoint to retrieve the schema of a collection. Uses pydantics model_json_schema() method.
        ...
        async def post_endpoint(
            ...
            for item in data:
                try: 
                    ...
                    try:
                        # TODO Check if field is read only
                        ...
                        pass
                    # TODO Execute hook 'pre_query_callback' here
                    row = collection(**item.model_dump()) # Validate the data
                    _hook = self._execute_callback(input_hook, query=CRUDQueryData(data=item.model_dump(), method="post",request=request), output=row)
                    if _hook: row = _hook
                    _to_insert.append(row.model_dump())
                except pymongo_errors.PyMongoError as e:
            ...
            try:
                if _to_insert:
                    log.warning("Inserting..." + str(collection.Settings.name) + str(_to_insert))
                    _collection = Database.get_collection(collection.Settings.name)
                    # insert = await collection.insert_many(_to_insert) 
                    insert = _collection.insert_many(_to_insert)
                    log.warning(insert)
                    query = {"_id": {"$in": insert.inserted_ids}}
                    # documents = await collection.find(query).to_list(None)
                    documents = _collection.find(query)
                    completed = [output_model(**item).model_dump() for item in documents]
        ...
        # Assuming input_model is your original Pydantic model
        # OptionalInputModel = create_model(
        #     "OptionalInputModel",
        #     **{
        #         name: (Optional[type(field)], None)
        #         for name, field in input_model.__annotations__.items()
        #         if isinstance(field, type)
        #     },
        #     __config__=create_model("Config", arbitrary_types_allowed=(bool, True)),
        #     # __base__=DatabaseMongoBaseModel
        # )   
        # # print(type(OptionalInputModel), OptionalInputModel.model_json_schema())
        # # print(input_model.__annotations__.items())
        # print([type(field) for name, field in input_model.__annotations__.items()])
        # print({name: (Optional[type(field)], None)
        #         for name, field in input_model.__annotations__.items()
        #         })
        ...
        async def patch_many_endpoint( # DEBUG
                request: Request,
                data: Union[List[input_model], input_model],
                # data: List[OptionalInputModel],
            ): 
        ...
        for _method in method:
            ...
            match _method.lower():
                case "get":
                    ...
                    if name_plural:
                        self.router.add_api_route(
                            ...
                            tags=tags,
                            summary=f"Get multiple {(self.base_name + ' ' if self.base_name else '')}{name_plural}",
                            description=description ,
                            include_in_schema=include_in_schema,
                    ...
                    if name_singluar:
                        ...
                        self.router.add_api_route(
                            ...
                            response_model=dict,
                            tags=tags,
                            summary=f"Get schema for {(self.base_name + ' ' if self.base_name else '')}{name_singluar.capitalize()}",
                        ...
                        self.router.add_api_route(
                            ...
                            tags=tags,
                            summary=f"Get one {(self.base_name + ' ' if self.base_name else '')}{name_singluar.capitalize()} by ID",
                            description=description,
                            include_in_schema=include_in_schema,
                ...
                case "post":
                    if name_singluar:
                        self.router.add_api_route(
                            ...
                            tags=tags,
                            summary=f"Create one/multiple {(self.base_name + ' ' if self.base_name else '')}{name_singluar.capitalize()}",
                ...
                case "patch":
                    ...
                    #         response_model=CRUDResponseModelPostMany,
                    #         tags=tags,
                    #         summary=f"Update one {(self.base_name + ' ' if self.base_name else '')}{name_singluar.capitalize()}",
                    ...
                    if name_plural:
                        self.router.add_api_route(
                            ...
                            response_model=CRUDResponseModelPostMany,
                            tags=tags,
                            summary=f"Update many {(self.base_name + ' ' if self.base_name else '')}{name_plural.capitalize()}",
                ...
                case "delete":
                    if name_singluar:
                        self.router.add_api_route(
                            ...
                            tags=tags,
                            summary=f"Delete one {(self.base_name + ' ' if self.base_name else '')}{name_singluar.capitalize()}",
